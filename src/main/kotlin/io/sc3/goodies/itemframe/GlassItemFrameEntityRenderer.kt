...
class GlassItemFrameEntityRenderer(
  ...
  private fun renderItemStack(entity: GlassItemFrameEntity, matrices: MatrixStack, consumers: VertexConsumerProvider,
    ...
    val mapId = entity.getMapId(stack);
    ...
    if (entity.containsMap()) {
      ...
      if (stack.item is BannerItem) {
        ...
        BannerBlockEntityRenderer.renderCanvas(matrices, consumers, light, DEFAULT_UV, bannerModel, ModelLoader.BANNER_BASE, true, color, patterns)
        matrices.pop()
      } else {
  ...
  override fun hasLabel(entity: GlassItemFrameEntity) =
    if (MinecraftClient.isHudEnabled()
      ...
      && entity.heldItemStack.getName().string.trim().isNotEmpty()
